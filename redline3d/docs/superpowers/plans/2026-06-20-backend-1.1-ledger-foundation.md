...
**Architecture:** A standalone `server/` Fastify app (sibling of `redline3d/`). Money is an **append-only ledger** (`ledger_entries`): balance is the sum of deltas, never a mutable field — the correct, auditable design for real value later. All amounts are **integer coins** (no floats). Per-user balance mutations are serialized with a Postgres advisory lock to prevent overdraft races. Auth is a swappable `requireUser` preHandler that today resolves a user from a dev header and later (plan 1.3) verifies a Privy token. Tests run against in-process **PGlite** (real Postgres semantics, no Docker) so the TDD loop is instant.
...
> NOTE: merge the new imports (`bigint`, `index`) into the existing `from "drizzle-orm/pg-core"` import line rather than adding a second import statement.
...
export function makeLedger(db: any) {
  return {
    ...
    /** low-level append. delta may be + or -. Prefer credit()/debit(). */
    async post(userId: string, delta: number, reason: string, ref?: string): Promise<void> {
      await db.insert(ledgerEntries).values({ userId, delta, reason, ref: ref ?? null });
    },

    async credit(userId: string, amount: number, reason: string, ref?: string): Promise<void> {
      assertPositiveInt(amount, "credit amount");
      await db.insert(ledgerEntries).values({ userId, delta: amount, reason, ref: ref ?? null });
    },
  };
}
...
```ts
    ...
    async debit(userId: string, amount: number, reason: string, ref?: string): Promise<void> {
      ...
      await db.transaction(async (tx: any) => {
        ...
        await tx.insert(ledgerEntries).values({ userId, delta: -amount, reason, ref: ref ?? null });
      });
    },
```
...
- [ ] **Step 2: Create `server/Procfile`** (Railway start command)
...
- Start command: `npm run start` (migrations run on boot).
````
...
**Known follow-ups (next plans):** advisory-lock concurrency is real only under postgres.js (pglite is single-connection) — exercise it in plan 1.2 load tests; idempotency via unique `ref` index lands with crates (1.4).

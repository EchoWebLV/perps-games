...
- Modify: `/Users/yordanlasonov/.claude/projects/-Users-yordanlasonov-Documents-GitHub-perps-games/memory/perps-rider-product.md` (+ `MEMORY.md` index if the hook line changes)
...
- [ ] **Step 2: Update memory** — in `perps-rider-product.md`, append that Plan 1.2 is built (link `[[redline-economics]]` for the linear-from-entry formula the server now enforces). Keep it one fact; update the index line in `MEMORY.md` if the hook changes.

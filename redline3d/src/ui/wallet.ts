...
export function createWallet(parent: HTMLElement, opts: WalletOpts): Wallet {
  ...
  panel.innerHTML =
    ...
    `<div class="wlt-hero"><div class="wlt-hero-glow"></div>
       <div class="wlt-hero-top">${usdcCoin(22)}<span class="wlt-hero-lbl">USDC Balance</span></div>
    ...
    `<div class="wlt-view" data-view="recv" hidden>
  ...
  copyBtn.onclick = async () => {
    try { await navigator.clipboard?.writeText(opts.address); } catch { /* clipboard unavailable */ }
  ...
  const setOpen = (open: boolean) => {
    ...
    if (open) { renderBalance(); showTab("buy"); }
  };
  overlay.onclick = (e) => {

// This file is a deliberate COPY of programs/raider/src/price.rs, not a shared
// module: the decoder has no dependencies, and coupling two independently
// -deployed programs through a shared crate would force a raider redeploy
// every time paddock changes. Only the `RaiderError` -> `PaddockError`
// references were changed to compile against this crate; the decoding logic
// and EXPECTED_FEED_OWNER are byte-identical to the raider original.
//

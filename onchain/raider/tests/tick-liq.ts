...
// Pin the base-layer WS endpoint. web3.js auto-derives the WS URL from BASE_RPC,
// but some HTTP providers (e.g. Helius) serve a signature-subscription stream that
// rpc-websockets v9 cannot parse ("Unknown action 'undefined'"), which makes
// Anchor's .rpc() throw even though the tx lands. Pinning a known-good devnet WS
// (the public one) keeps Anchor's confirmation path working while HTTP sends go to
// whatever (possibly unthrottled) BASE_RPC is configured.
...
// Send + confirm a built Anchor instruction via HTTP polling ONLY (no WS).
// The base-layer `delegate_session` CPI is heavy and its WebSocket confirmation

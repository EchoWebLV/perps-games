...
describe("makeEvmTreasurySigner", () => {
  ...
  it("sends to the requested destination and signs as the treasury account on the configured chain", async () => {
    const { wallet, writes } = makeWallet();
    ...
    expect(writes[0].account).toBe(wallet.account);
    expect(writes[0].chain).toBe(wallet.chain);
  });
